...
import OrionBackground from './components/OrionBackground';
import BrandBackground from './components/BrandBackground';
import Logo from './components/Logo';
import AdminLogin from './components/AdminLogin';
import RunthroughWizard from './components/RunthroughWizard';
...
import { 
  ...
  Loader2, History, Lock, Radio, Clock, Building2, Box, Rocket, ShieldAlert, RefreshCcw
} from 'lucide-react';
...
class GlobalErrorBoundary extends Component<{children: ReactNode}, {hasError: boolean}> {
  ...
  componentDidCatch(error: Error, errorInfo: ErrorInfo) { console.error("Global Protocol Fault:", error, errorInfo); }
  render() {
    if (this.state.hasError) {
      return (
        <div className="min-h-screen bg-slate-950 flex items-center justify-center p-8 text-center">
          <div className="max-w-md w-full space-y-8 p-12 bg-slate-900 border border-red-500/20 rounded-[3rem] shadow-2xl">
            <div className="p-4 bg-red-500 rounded-3xl text-white w-fit mx-auto shadow-lg"><ShieldAlert className="w-10 h-10" /></div>
            <div className="space-y-2">
              <h2 className="text-2xl font-black text-white uppercase tracking-tight">Circuit Breaker Active</h2>
              <p className="text-slate-400 text-sm">The application encountered a structural data fault. Neural core halted to prevent data corruption.</p>
            </div>
            <button 
              onClick={() => window.location.reload()}
              className="w-full py-4 bg-white text-slate-950 rounded-2xl font-black uppercase text-xs tracking-widest flex items-center justify-center gap-2 hover:bg-blue-50 transition-all"
            >
        ...
        <div className="max-w-4xl w-full relative z-10 flex flex-col items-center text-center">
          ...
          <div className="mt-24 flex flex-wrap justify-center gap-12 text-slate-500 animate-in fade-in duration-1000 delay-1000">
            ...
            <div className="flex flex-col items-center gap-2">
              ...
              <span className="text-xs font-bold text-white uppercase">v2.5.5 Stable</span>
            </div>
            <div className="w-px h-8 bg-slate-800" />
            <div className="flex flex-col items-center gap-2">
